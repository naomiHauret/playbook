...
task('e2e', 'Runs all e2e tests')
  ...
  .setAction(async (taskArgs: any, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      testResults['Anthropic claude-3-5-sonnet-20240620'] = result.error
        ? truncateMessage(result.error)
        : '✅'
    } catch (e: any) {
...
async function runTaskWithTimeout(
  ...
  try {
    ...
    const taskResult = await Promise.race([timeoutPromise, hre.run(taskIdentifier, taskArguments)])
    return taskResult
  } catch (e: any) {
