...
async function runTests(
  ...
  try {
    ...
    await postSlackMessage(e.message, slackWebHookUrl)
  }

  // If isSuccess is false, you might want to send a message to Slack here as well
  if (!isSuccess || (isSuccess && !previousResult)) {
    await postSlackMessage(stdoutData.length > 0 ? stdoutData : stderrData, slackWebHookUrl)
  }

  return isSuccess
}

async function postSlackMessage(output: string, webhookUrl: string) {
  try {
