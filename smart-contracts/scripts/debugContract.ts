...
async function deployContract(
  ...
  */
  ...
  try {
    ...
    throw Error(
      `Failed to deploy contract, please ensure the constructor parameters are correct: [${constructorArgs}]`,
    )
  }
}
...
async function addOracleResponse(
  ...
  if (error) {
    try {
      ...
      await oracle
        ...
        .addOpenAiResponse(promptId, promptCallbackId, openAiResponse, '')
      error = null
    } catch (e) {
      error = e
    }
  ...
  if (error) {
    try {
      ...
      await oracle
        .connect(oracleAccount)
        .addGroqResponse(promptId, promptCallbackId, groqResponse, '')
      error = null
    } catch (e) {
      error = e
    }
...
describe('Contract', function () {
  ...
  describe('Basic usage', function () {
    it('Can start chat', async () => {
      ...
      expect(messages[0].message).to.not.equal(
        null,
        `Expected the first message from the "getMessages" function not to be null`,
      )
    })
    it('Oracle can add response', async () => {
      const { chatGpt, oracle, owner, allSigners } = await loadFixture(deploy)
      ...
      expect(messages[messages.length - 1].message).to.equal(
        'Hi',
        'Expected the last message to be equal to the last message sent in',
      )
    })
  ...
  describe('Error handling', function () {
    it('User cannot start chat and add another message', async () => {
      ...
      expect(error).to.not.be.equal(
        null,
        'Expecting add message to fail if called multiple times in a row',
      )
    })
