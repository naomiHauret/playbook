...
async function deployAgent(oracleAddress: string) {
  const agent = await ethers.deployContract('Agent', [oracleAddress, AGENT_PROMPT], {})
...
async function deployDalle(oracleAddress: string) {
  const agent = await ethers.deployContract('DalleNft', [oracleAddress, DALLE_PROMPT], {})
...
async function deployVitailik(oracleAddress: string) {
  const agent = await ethers.deployContract('Vitailik', [oracleAddress, VITAILIK_PROMPT], {})
