...
describe('Vitailik', function () {
  ...
  async function deploy() {
    ...
    const agent = await Vitailik.deploy(oracle.target, PROMPT)
  ...
  describe('Parsing', function () {
    ...
    it('Should parse out image', async () => {
      ...
      // expect(await agent.oracleAddress()).to.equal("0x0000000000000000000000000000000000000000");
      const imageLine =
        '<IMAGE A futuristic digital coliseum, filled with neon lights and a center stage where VIILIK, a grotesque man-dog hybrid with crypto tattoos, stands wielding bizarre weapons.'
  ...
  describe('Errors', function () {
    it('Cannot add multiple responses', async () => {
      ...
      await expect(
        agent.connect(randomAddress).onOracleGroqLlmResponse(
          0,
          {
            id: '123',
    ...
    it('Cannot add selection for finished game', async () => {
      ...
      await expect(agent.addSelection(3, 0)).to.be.revertedWith('Game is finished')
    })
    it('Cannot add invalid selection', async () => {
      ...
      await expect(agent.addSelection(8, 0)).to.be.revertedWith('Selection needs to be 0-3')
    })
  })
})
