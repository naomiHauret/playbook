...
describe('ChatOracle', function () {
  ...
  describe('Deployment', function () {
    ...
    it('Only whitelisted address can update attestation', async () => {
      ...
      await expect(oracle.connect(allSigners[1]).addAttestation('attestation')).to.be.rejectedWith(
    ...
    it('Only owner can update pcr0 hash', async () => {
      ...
      await expect(oracle.connect(allSigners[1]).addPcr0Hash('hash')).to.be.rejectedWith(
        'Caller is not owner',
      )
    })
  })
