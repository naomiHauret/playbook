...
contract Vitailik {
    ...
    function startGame() public returns (uint) {
        ...
        assistantMessage
            .content = "I'll first describe the scene and then give 4 options (a,b,c,d) for player to choose what character to play as. I'll come up with animals to select from, and add 2 adjectives to combine as the character name. I'll keep it short. I'll structure it as following:\n\n<short scene description>\n\n<4 character choices>\n\n<image description>\n\n<HPs>";
    ...
    function onOracleGroqLlmResponse(
        ...
        require(
            compareStrings(
                game.messages[game.messagesCount - 1].role,
                'user'
            ) || game.messagesCount == 3,
        ...
        if (
            compareStrings(hpValues[0], '0') || compareStrings(hpValues[1], '0')
        ) {
    ...
    function findHPInstances(
        string memory input
    ) public pure returns (string[2] memory) {
        bytes memory inputBytes = bytes(input);
        ...
        for (uint i = 0; i < inputBytes.length; i++) {
            ...
            if (
                ...
                while (
                    ...
                    ((inputBytes[endIndex] >= '0' &&
                        inputBytes[endIndex] <= '9') ||
                        inputBytes[endIndex] == ',')
                ) {
                    endIndex++;
                }
